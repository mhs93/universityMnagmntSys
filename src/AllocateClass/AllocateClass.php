...
:room_id, :day_id, :start, :end )";
    ...
    public function getScheduleById($id=""){
        $this->id = $id;
        $sql = "SELECT * FROM allocate_rooms WHERE id='$this->id'";
        $stmt = Database::Prepare($sql);
        $stmt->execute();
        $data =  $stmt->fetch();
        return $data;
    }
    public function scheduleUpdate($data, $id=""){
        ...
        $sql = "UPDATE allocate_rooms SET dept_id=:dept_id, course_id=:course_id, room_id=:room_id, day_id=:day_id, start=:start, end=:end WHERE id=:id";
        $stmt = Database::Prepare($sql);
        $data = [
            ':dept_id'=>$this->deptId,
            ':course_id' =>$this->courseId,
        ...
        $status = $stmt->execute($data);
        if ($status){
            $_SESSION["SuccessMsg"] = "Schedule is Successfully Updated.";
        }else{
            $_SESSION["ErrorMsg"] = "Schedule update failed.";
        }
    }
