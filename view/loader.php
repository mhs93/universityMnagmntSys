...
if (isset($_POST["depart_id"])){
    if ($_POST["depart_id"] != ''){
        $sql= "SELECT students.*,departments.code FROM students LEFT JOIN departments on students.dept_id = departments.id WHERE dept_id = '".$_POST["depart_id"]."' AND students.status='1' ORDER BY id DESC";
    }
    else{
...
if (isset($_POST["department_id"])){
    $sql= "SELECT * FROM teachers WHERE dept_id = '".$_POST["department_id"]."' AND status=1";
    $stmt = Database::Prepare($sql);
    $stmt->execute();
    $data =  $stmt->fetchAll();
...
if (isset($_POST["deptfor_crs"])){
    $sql= "SELECT * FROM courses WHERE dept_id = '".$_POST["deptfor_crs"]."' AND status=1";
    $stmt = Database::Prepare($sql);
    $stmt->execute();
    $data =  $stmt->fetchAll();
    $output = '<option value="">&larr; Select Course Name &larr;</option>';
    foreach ($data as $value){
...
if (isset($_POST["deptfor_code"])){
    $sql= "SELECT * FROM courses WHERE dept_id = '".$_POST["deptfor_code"]."' AND status=1";
    $stmt = Database::Prepare($sql);
    $stmt->execute();
    $data =  $stmt->fetchAll();
    $output = '<option value="">&larr; Select Course Code &larr;</option>';
    foreach ($data as $value){
        $output .= '<option value="'.$value["id"].'">'.$value["course_code"].'</option>';
    }
    echo $output;
}
...
departments.id WHERE students.id = '".$_POST["stdId"]."'";
    ...
    $output .= '<input type="text" class="form-control" name="department" readonly="readonly" value="'.$data["code"]
        .'"><br>';
    $query = "SELECT * FROM courses WHERE dept_id = '".$data["id"]."' AND status=1";
...
departments.id WHERE students.id = '".$_POST["result"]."'";
    ...
    $output .= '<input type="text" class="form-control" name="department" readonly="readonly" value="'.$data["code"]
        .'"><br>';
    $output .= '<a  class="btn btn-adn pull-right" href="pdf.php?pdfId='.$_POST["result"].'">Make PDF</a><br>';
    echo $output;
}

if (isset($_POST["grade"])){
    $query = "SELECT courses.id, courses.course_name, courses.course_code FROM entrol_course LEFT JOIN courses 
ON entrol_course.course_id=courses.id  WHERE entrol_course.reg_no = '" .$_POST["grade"]."'";
    $stmt = Database::Prepare($query);
    $stmt->execute();
    $data =  $stmt->fetchall();
    foreach ($data as $results) {
        ...
        if ($totalRows < 1){
            $output .='<td>Not Graded Yet</td>';
            $output .='<td><a class="btn btn-default" href="result2.php?resultId='.$_POST["grade"].'">Create Now</a></td>';
        }else {
            foreach ($gradeLetter as $grade) {
                $output .='<td>' . $grade['grade'] . '</td>';
        $output .='<td>';
        $output .= '<a  class="btn btn-default" href="edit-result.php?editGrade='.$grade["id"].'">Edit</a>';
        $output .= '</td>';
            }
        }
        $output .='</tr>';
...
if (isset($_POST["enrollId"])){
    if ($_POST["enrollId"] != ''){
        $sql = "SELECT students.reg_no, students.title, courses.course_name, departments.code,entrol_course.date,entrol_course.id FROM entrol_course LEFT JOIN students ON entrol_course.reg_no=students.id LEFT JOIN courses ON entrol_course.course_id=courses.id LEFT JOIN departments ON students.dept_id=departments.id WHERE students.dept_id = '".$_POST["enrollId"]."' AND students.status='1' ORDER BY entrol_course.id DESC ";
    }
    else{
...
.course_id=courses.id LEFT JOIN departments ON students.dept_id=departments.id WHERE students.status='1' ORDER BY entrol_course.id DESC ";
    }
...
if (isset($_POST["dpt_teacher"])) {
    if ($_POST["dpt_teacher"] != '') {
        $sql = "SELECT teachers.id, teachers.name, teachers.address,teachers.email, teachers.contact, teachers.total_credit, teachers.remaining_credit, designations.title, departments.code FROM teachers LEFT JOIN designations ON teachers.desig_id=designations.did LEFT JOIN departments ON teachers.dept_id=departments.id WHERE departments.id = '" . $_POST["dpt_teacher"] . "' AND teachers.status=1 ORDER BY teachers.id DESC ";
    } else {
        $sql = "SELECT teachers.id, teachers.name, teachers.address,teachers.email, teachers.contact, teachers.total_credit, teachers
.remaining_credit, designations.title, departments.code FROM teachers LEFT JOIN designations ON teachers
.desig_id=designations.did LEFT JOIN departments ON teachers.dept_id=departments.id WHERE teachers.status=1 ORDER BY teachers
.id DESC ";
    }
    ...
    foreach ($data as $val){
        ...
        if ($val["remaining_credit"] <= 0){
            $output .= '<td>';
            $output .='<span style="color: #ac2925; font-weight: bold">' . $val["remaining_credit"] . '</span>';
            $output .='</td>';
...
if (isset($_POST["scheduleId"])){
    ...
    if ($_POST["scheduleId"] != ''){
        $sql = 'SELECT * FROM courses WHERE dept_id = "'.$_POST["scheduleId"].'" AND courses.status=1 ';
    }else{
        $sql = 'SELECT * FROM courses WHERE courses.status=1 ';
    }
    ...
    foreach ($courseDetail as $data){
        ...
        $output .= '<td>';
            ...
            $sql = 'SELECT rooms.room_no, days.title,allocate_rooms.id, allocate_rooms.start, allocate_rooms.end FROM allocate_rooms LEFT JOIN rooms ON allocate_rooms.room_id=rooms.id LEFT JOIN days ON allocate_rooms.day_id=days.id WHERE allocate_rooms.course_id = "'.$data["id"] .'" AND value=1';
            $stmt = Database::Prepare($sql);
            $stmt->execute();
            $totalRows =  $stmt->rowCount();
            ...
            if ($totalRows < 1){
                ...
                foreach ($schedules as $schedule) {
                    $output .= "Room No : " . $schedule["room_no"] . ", " . $schedule["title"] . " , " .
                        date("g:i a", strtotime($schedule["start"])) . " - " . date("g:i a", strtotime($schedule["end"])) ."<a href='editSchedule.php?editSchedule=" .$schedule["id"]. "'> Edit</a>". "<br>";
                }
            }
