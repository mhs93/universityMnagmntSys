        ...
        <div class="box box-default">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (isset($_POST["submit"])) {
                    ...
                    if (!empty($_POST["courseId"])){
                        ...
                        $sql = "SELECT * FROM allocate_rooms WHERE course_id = '$courseId' AND day_id = '$dayId'AND value=1 ";
                    ...
                    if (!empty($_POST["start"]) && !empty($_POST["end"])){
                        ...
                        $sql = "SELECT allocate_rooms.*, rooms.room_no, days.title, courses.course_code FROM allocate_rooms LEFT JOIN rooms on allocate_rooms.room_id = rooms.id LEFT JOIN days ON allocate_rooms.day_id = days.id LEFT JOIN courses ON allocate_rooms.course_id=courses.id WHERE allocate_rooms.room_id = '$roomId' AND allocate_rooms.day_id = '$dayId' AND value = 1";
                        $stmt = Database::Prepare($sql);
                        $stmt->execute();
                        $data =  $stmt->fetchAll();
                        $i = 0;
            ...
            <div class="box-body">
                <form action="" method="post" class="form-horizental col-sm-offset-4 col-sm-4 col-sm-offset-4">
                    ...
                    <a href="view-schedule.php" class="btn btn-info">View Schedule</a>
                </form>
            </div>
        </div>
    </section>
